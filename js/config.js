...
// ── localStorage keys (used by setup.html and config helpers) ──────────────
const CFG_KEYS = {
...
/** Persists user-supplied config to localStorage */
...
/** Wipes all config from localStorage (triggers re-setup on next page load) */
function clearConfig() {
  Object.values(CFG_KEYS).forEach(k => localStorage.removeItem(k));
}

/** @type {Object} Global application configuration — personal values read from localStorage */
const CONFIG = {
